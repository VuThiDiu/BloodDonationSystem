...
def make_request_view(request):
    ...
    if request.method=='POST':
        ...
        if request_form.is_valid():
            ...
            blood_request.hospitalID=request_form.data['hospitalID']
            blood_request.save()
...
def event_view(request):
    req=orgamodels.RequestBloodDonationRequest.objects.all().filter(status="Approved")
    return render(request,'donor/event_organization.html',{'requests':req})

def other_request_view(request):
    donor= models.Donor.objects.get(user_id=request.user.id)
    blood_request=bmodels.BloodRequest.objects.all().exclude(request_by_donor=donor).filter(status="Approved")
    return render(request,'donor/other_request.html',{'blood_request':blood_request})
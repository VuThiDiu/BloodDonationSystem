...
from organization import models as orgamodels
from donor import forms as dforms
from patient import forms as pforms
from hospital import forms as hfroms
from organization  import forms as orgaforms
...
def admin_hospital_view(request):
    hospital=hmodels.Hospital.objects.all()
    return render(request,'blood/admin_hospital.html', {'hospitals':hospital} )
...
def admin_request_view(request):
    requests=models.BloodRequest.objects.all().filter(status_hospital='Approved').filter(status='Pending')
    return render(request,'blood/admin_request.html',{'requests':requests})


def update_approve_status_from_hospital_view(request,pk):
    request=models.BloodRequest.objects.get(id=pk)
    request.status_hospital="Approved"
    request.save()
    return HttpResponseRedirect('/hospital/process-request')

def update_reject_status_from_hospital_view(request,pk):
    request=models.BloodRequest.objects.get(id=pk)
    request.status_hospital="Rejected"
...
@login_required(login_url='adminlogin')
def admin_request_hospital_view(request):
    requests=models.BloodRequestByHospital.objects.all().filter(status='Pending')
    return render(request,'blood/admin_request_hospital.html',{'requests':requests})
...
@login_required(login_url='adminlogin')
def admin_request_history_hospital_view(request):
    requests=models.BloodRequestByHospital.objects.all().exclude(status='Pending')
    return render(request,'blood/admin_request_history_hospital.html',{'requests':requests})    

@login_required(login_url='adminlogin')
def admin_request_organization_view(request):
    requests= orgamodels.RequestBloodDonationRequest.objects.all().filter(status='Pending')
    return render(request,'blood/admin_request_organization.html',{'requests':requests})
...
@login_required(login_url='adminlogin')
def update_approve_status_hospital_view(request,pk):
    req=models.BloodRequestByHospital.objects.get(id=pk)
    message=None
    bloodgroup=req.bloodgroup
    unit=req.unit
    stock=models.Stock.objects.get(bloodgroup=bloodgroup)
    ...
    requests=models.BloodRequestByHospital.objects.all().filter(status='Pending')
    return render(request,'blood/admin_request_hospital.html',{'requests':requests,'message':message})

@login_required(login_url='adminlogin')
def update_reject_status_organization_view(request,pk):
    req=orgamodels.RequestBloodDonationRequest.objects.get(id=pk)
    req.status="Rejected"
    req.save()
    return HttpResponseRedirect('/admin-request-organization')


@login_required(login_url='adminlogin')
def update_approve_status_organization_view(request,pk):
    req=orgamodels.RequestBloodDonationRequest.objects.get(id=pk)
    req.status="Approved"
    req.save()
    return HttpResponseRedirect('/admin-request-organization')
...
@login_required(login_url='adminlogin')
def update_reject_status_hospital_view(request,pk):
    req=models.BloodRequestByHospital.objects.get(id=pk)
    req.status="Rejected"
    req.save()
    return HttpResponseRedirect('/admin-request-hospital')
